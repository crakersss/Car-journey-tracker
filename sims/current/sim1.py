...
class OBDGui(QWidget):
    ...
    def init_setup_page(self):
        ...
        instructions_label = QLabel(
            f"Enter your car's maximum RPM ({self.min_rpm}–{self.max_rpm_limit}) "
            f"and speed ({self.min_speed}–{self.max_speed_limit} km/h) in the fields below."
        )
    ...
    def init_main_page(self):
        ...
        # Button for starting/stopping CSV logging
        self.log_button = QPushButton("Start Logging")
        self.log_button.clicked.connect(self.toggle_logging)
        main_layout.addWidget(self.log_button, alignment=Qt.AlignmentFlag.AlignCenter)
    ...
    def update_status(self):
        if self.engine_stalled:
            self.status_label.setText(
                "💀 Engine Stalled! Press R to Restart (Hold Enter: Throttle, Space: Clutch, W: Up, S: Down)"
            )
        elif self.is_logging:
            self.status_label.setText(f"Logging to CSV (Hold Enter: Throttle, Space: Clutch, W: Up, S: Down)")
        else:
            self.status_label.setText(
                "🚗 Simulation Mode ON (Hold Enter: Throttle, Space: Clutch, W: Gear Up, S: Gear Down)"
            )

    def toggle_logging(self):
        """Toggle CSV logging on/off, prompting for file location when starting."""
        if not self.is_logging:
            # Prompt for file location using QFileDialog
            default_filename = f"sim_data_{time.strftime('%Y%m%d_%H%M%S')}.csv"
            ...
            if filepath:
                ...
                try:
                    ...
                    self.csv_writer.writerow(
                        ["timestamp", "rpm", "speed", "throttle", "temp", "load", "boost", "gear", "lat", "lon"]
                    )
                    self.csv_file.flush()
                    ...
                    QMessageBox.critical(
                        self,
                        "Error",
                        "Permission denied: Cannot write to the selected location. Choose a different directory with write access."
                    )
            ...
            if self.csv_file:
                try:
                    ...
                    QMessageBox.warning(self, "Warning", f"Error closing file: {e}")
                self.csv_file = None
                self.csv_writer = None
            self.is_logging = False
    ...
    def fast_update(self):
        """Update simulation data and log to CSV if enabled."""
        if self.engine_stalled:
            self.current_rpm = 0
            self.current_speed = 0
            self.sim_throttle = 0
            ...
            self.update_display()
            # Log stalled state
            if self.is_logging and (time.time() - self.last_log_time) >= self.log_interval:
                self.log_data()
            return
        ...
        # Log data to CSV if enabled and interval reached
        if self.is_logging and (time.time() - self.last_log_time) >= self.log_interval:
            self.log_data()
    ...
    def log_data(self):
        ...
        if self.csv_writer:
            try:
                ...
                # Mock GPS coordinates (replace with NEO-6M data later)
                lat, lon = 0.0, 0.0
                self.csv_writer.writerow(
                    [
                        f"{timestamp:.3f}",
                        int(self.current_rpm),
                        int(self.current_speed),
                        int(self.current_throttle),
                        int(self.current_temp),
                        int(self.current_load),
                        f"{self.current_boost:.1f}",
                        self.current_gear,
                        lat,
                        lon,
                    ]
                ...
                QMessageBox.warning(self, "Warning", f"Error writing to CSV: {e}")
    ...
    def reconfigure(self):
        ...
        # Stop logging when reconfiguring
        if self.is_logging:
            self.toggle_logging()
